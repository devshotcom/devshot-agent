...
# The watchdog is implemented by the same signed Go binary but runs as an
# independent systemd process, so it survives a killed or stopped agent.
install -m 0644 "$SCRIPT_DIR/devshot-host-watchdog.service" "$ROOTFS/etc/systemd/system/devshot-host-watchdog.service"
install -m 0644 "$SCRIPT_DIR/devshot-host-watchdog.timer" "$ROOTFS/etc/systemd/system/devshot-host-watchdog.timer"
